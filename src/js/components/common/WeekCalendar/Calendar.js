...
import {getMonthLocale, getWeekLocale, getWeekNumber, getYearLocale} from './util/utils';
...
class Calendar extends Component {
    ...
    translateToStart() {
        if (this.state.shouldTranslate) {
            ...
            translateUtil.translateElement(wrapper, this.valueToTranslate(startDateAt), null);
            this.setState({
    ...
    initScrollEvent() {
        ...
        draggable(el, {
            ...
            drag: (event) => {
                ...
                var translateY = dragState.startTranslateTop + deltaY;
                translateUtil.translateElement(el,translateX, 0);
            },
            end: (touches) => {
                ...
                if (this.state.dragging) {
                    if(this.props.onDragEnd){
                        var deltaY = dragState.top - dragState.startTop;
                        var deltaX = dragState.left - dragState.startLeft;
                        this.props.onDragEnd(deltaX,deltaY);
                    }
                    ...
                    setTimeout(() => {
                        ...
                        let pos = currentTranslate / itemWidth;
                        let rand = Math.round(pos);

                        if(pos-rand<-0.2){
                            translate = Math.floor(pos)  * itemWidth;
                        }
                        else if(pos-rand>0.2){
                            translate = Math.ceil(pos)  * itemWidth;
                        }
    ...
    calcDragRange() {
        ...
        const {scrollableData} = this.state;
        let itemWidth = this.getItemWidth();
        return [-itemWidth * (scrollableData.length - Math.ceil(1 / 2)), itemWidth * Math.floor(1 / 2)];
    }
    ...
    valueToTranslate(value) {
        const {view} = this.props;
        const itemHeight = this.getItemWidth();
    ...
    renderHeader() {
        ...
        return (
            <div className="react-calendar__control">
                <div className="react-calendar__arrow react-calendar__arrow--left"
                     onClick={this.changeToPrevMonthOrWeek}></div>
                ...
                <div className="react-calendar__arrow react-calendar__arrow--right"
                     onClick={this.changeToNextMonthOrWeek}></div>
            </div>
        );
    }
    ...
    render() {
        ...
        let itemWidth = this.getItemWidth();
        const contentWidth = itemWidth * scrollableData.length;
        ...
        return (
            <div className={wrapClass}>
                {this.props.view=='month' &&<div className="react-calendar__header">
                    {this.renderHeader()}
                </div>}
                <div ref={(main) => this.main = main} className="react-calendar__main"
                     style={{height: `${ITEM_HEIGHT[view] + mainDivHeight}px`}}>
                    <div className="react-calendar__weekdays">
                        {this.renderWeekTitle()}
                    </div>
                    <div ref={(wrapper) => this.wrapper = wrapper} className={daysWrapClass}
